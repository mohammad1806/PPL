...
import { isTVar, makeFreshTVar, parseTExp, unparseTExp, TExp, makeBoolTExp, makeNeverTExp } from './TExp';
...
import { Result, bind, makeFailure, mapResult, makeOk, mapv, isOk } from "../shared/result";
...
export type ProcExp = {tag: "ProcExp"; args: VarDecl[], body: CExp[]; returnTE: TExp , ifPredTckeck : TExp }
export const makeProcExp = (args: VarDecl[], body: CExp[], returnTE: TExp, ifPredTckeck : TExp): ProcExp =>
    ({tag: "ProcExp", args: args, body: body, returnTE: returnTE, ifPredTckeck : ifPredTckeck});
...
const parseProcExp = (vars: Sexp, rest: Sexp[]): Result<ProcExp> => {
    if (isArray(vars)) {
        ...
        if( rest[0] === ":" && rest[1] === "is?"){
            if( vars.length != 1 ){
                return makeFailure(`type predicate should recives just one argument`)
            }
            const body = mapResult(parseL5CExp,rest.slice(3));
            const check = parseTExp(rest[2]);
            const returnTE = makeOk(makeBoolTExp());
            return bind(args, (args: VarDecl[]) =>
                bind(body, (body: CExp[]) =>
                    mapv(returnTE, (returnTE: TExp) =>
                        makeProcExp(args, body, returnTE, isOk(check) ? check.value : makeNeverTExp() ))));
        }
        else{
        ...
        return bind(args, (args: VarDecl[]) =>
                    bind(body, (body: CExp[]) =>
                        mapv(returnTE, (returnTE: TExp) =>
                            makeProcExp(args, body, returnTE, makeNeverTExp()))));
    }
    } else {
...
const unparseProcExp = (pe: ProcExp): Result<string> =>{
    // const vars = mapResult(unparseVarDecl,pe.args);
    // const body = unparseLExps(pe.body)
    // const returnT = unparseReturn(pe.returnTE)
    // if(!pe){
    //     return bind(vars , (vars : String[]) => 
    //         bind(returnT , (ret : string) => 
    //         mapv(body , (body : string) => `(lambda (${join(" ", vars)})${ret} ${body})`)))
    // }
    // else{
    //     return bind(vars , (vars : String[]) => 
    //         bind(returnT , (ret : string) => 
    //         mapv(body , (body : string) => `(lambda (${join(" ", vars)}) : is? ${ret} ${body})`)))
    //}
     return bind(mapResult(unparseVarDecl, pe.args), (vds: string[]) =>

...
import { isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
         ...
         makeNeverTExp,
         isNeverTExp,
         makeTypePredTExp, isTypePredTExp,
         ProceduresTExp,
         TypePredTExp,
         makeAnyTExp} from "./TExp";
...
const isTypePredApp = (e: Exp, tenv: TEnv): Result<AppExp> => {
    // return isAppExp(e) ?  isTypePredTExp(typeofExp(e.rator, tenv)) ? typeofExp(e.rator, tenv) : makeFailure(`not pred app`) 
    // : makeFailure(`not app exp`)
    if( isAppExp(e) ){
        return bind(typeofExp(e.rator , tenv) , (x : TExp) => isTypePredTExp(x) ? makeOk(e) : makeFailure(`not app pred`));
    }
    else{
        return makeFailure(`not app exp`);
    }
}

export const typeofIf = (ifExp: IfExp, tenv: TEnv): Result<TExp> =>
    either(
        bind (isTypePredApp(ifExp.test, tenv), (x : AppExp) => 
            bind( check(x, tenv), ( te : TypePredTExp ) => { 
            const thenTE = typeofExp(ifExp.then, isVarRef(x.rands[0]) ? makeExtendTEnv([x.rands[0].var],[te.checkIf],tenv) : tenv);
            const altTE = typeofExp(ifExp.alt, tenv);
            // L51
            const constraint2 = bind(thenTE, (thenTE: TExp) =>
                            bind(altTE, (altTE: TExp) =>
...
const check = (e: AppExp, tenv: TEnv): Result<TypePredTExp> => {
    // return isAppExp(e) ?  isTypePredTExp(typeofExp(e.rator, tenv)) ? typeofExp(e.rator, tenv) : makeFailure(`not pred app`) 
    // : makeFailure(`not app exp`)
    if( isAppExp(e) ){
        return bind(typeofExp(e.rator , tenv) , (x : TExp) => isTypePredTExp(x) ? makeOk(x) : makeFailure(`not app pred`));
    }
    else{
        return makeFailure(`not app exp`);
    }
...
export const typeofProc = (proc: ProcExp, tenv: TEnv): Result<TExp> => {
    if( !isNeverTExp(proc.ifPredTckeck) ){
        return makeOk(makeTypePredTExp(proc.args[0].texp, proc.ifPredTckeck))
    }
    else{

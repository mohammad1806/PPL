...
export type TExp =  AtomicTExp | CompoundTExp | TVar | neverTExp;
export const isTExp = (x: any): x is TExp => isAtomicTExp(x) || isCompoundTExp(x) || isTVar(x);

export type AtomicTExp = NumTExp | BoolTExp | StrTExp | VoidTExp | anyTExp ;
...
export const makeAnyTExp = () : anyTExp => 
    ({tag: "anyTExp"});
...
const flattenSortInter = (tes: TExp[]): TExp[] =>
    removeDuplicates(sort(superTypeComparator, flattenInter(tes)));
...
const normalizeInter = (ite: interTExp): TExp =>
    ...
    containsType(ite.components,makeAnyTExp()) ? makeInterTExp(filter(x => !isAnyTExp(x), ite.components)) :
...
export const makeDiffTExp =  (te1 : TExp, te2 : TExp) : TExp =>
    ...
    makeUnionTExp(filter(x => !containsType(te2.components,x), te1.components)) :
    isUnionTExp(te1) && isAtomicTExp(te2) && isSubType(te2, te1) ? makeUnionTExp(filter(x => !equals(x, te2),te1.components)) :
    te1; 


// typePredicate :--

export type ProceduresTExp = ProcTExp | TypePredTExp
export type TypePredTExp = {tag : "TypePredTExp", paramTE : TExp, checkIf : TExp}
export const isTypePredTExp = (x : any) : x is TypePredTExp => x.tag === "TypePredTExp";
export const makeTypePredTExp = (paramTE : TExp, checkIf : TExp) : TypePredTExp =>
    ({tag : "TypePredTExp", paramTE : paramTE, checkIf : checkIf})

const parseTypePredTExp = (Sexp : Sexp[]) : Result<TypePredTExp> =>{
    const pos = Sexp.indexOf('->')
    const posIs = Sexp.indexOf('is?')
    return (pos === -1)  ? makeFailure(`Procedure type expression without -> - ${format(Sexp)}`) :
           (pos > posIs) ? makeFailure(`Procedure type expression with -> after is? -  ${format(Sexp)}`) :
           (pos === 0) ? makeFailure(`No param types in proc texp - ${format(Sexp)}`) :
           (posIs === Sexp.length - 1) ? makeFailure(`No return type in proc texp - ${format(Sexp)}`) :
           (Sexp.slice(pos + 1).indexOf('->') > -1) ? makeFailure(`Only one -> allowed in a procexp - ${format(Sexp)}`) :
           bind(parseTExp(Sexp[0]), (param: TExp) =>
            mapv(parseTExp(Sexp[posIs + 1]), (checkIf: TExp) =>
                 makeTypePredTExp(param, checkIf)));
}
//=====================================================================================//
...
export const isSubType = (te1: TExp, te2: TExp): boolean =>
    ...
    isUnionTExp(te2) && isInterTExp(te1) ? filter(x => containsType(te2.components, x),te1.components).length > 0 :
...
export const unparseTExp = (te: TExp): Result<string> => {
    ...
    const up = (x?: TExp): Result<string | string[]> =>
        ...
        isTypePredTExp(x) ? bind(unparseTExp(x.paramTE), (paramTE: string) =>
                                mapv(unparseTExp(x.checkIf), (returnTE: string) =>
                                    [paramTE, '->', 'is?', returnTE])) :

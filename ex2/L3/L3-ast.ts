...
// new type : ClassExp --> asign 2.
export type ClassExp = {tag: "ClassExp"; feilds: VarDecl[]; methods: Binding[];}
...
// makeClassExp --> TODO 
export const makeClassExp = ( feilds: VarDecl[], methods: Binding[] ): ClassExp =>
    ({tag: "ClassExp", feilds: feilds, methods: methods});
...
const isSpecialForm = (x: string): boolean =>
    ["if", "lambda", "let", "quote", "class"].includes(x); //adding special form class
...
// seps has the shape (class (...))
export const parseClassExp = (feilds: Sexp, methods: Sexp): Result<ClassExp> =>{
    if (!(isArray(feilds) && allT(isString, feilds))) {
        return makeFailure(`Invalid feilds for classexp ${format(feilds)}`)
    }
    else if (!isGoodBindings(methods)) {
        return makeFailure('Malformed bindings in "class" expression');
    }
    else{
        const feildsNames = map(makeVarDecl, feilds);
        const methodsNames = map(b => b[0], methods);
        const methodsBodies = mapResult(parseL3CExp, map(second, methods));
        const resultBindings = mapv(methodsBodies, (vals: CExp[]) => zipWith(makeBinding, methodsNames, vals));
        return mapv(resultBindings, (x: Binding[]) => makeClassExp(feildsNames, x));
    }
}
...
// unparse class expression
const unparseClassExp = (cl: ClassExp) : string =>
    `(class (${map((p: VarDecl) => p.var, cl.feilds).join(" ")}) (${map((b: Binding) => `(${b.var.var} ${unparseL3(b.val)})`, cl.methods).join(" ")}))`
